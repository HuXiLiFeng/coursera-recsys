...
public class TFIDFItemScorer extends AbstractItemScorer {
	...
	public void score(long user, @Nonnull MutableSparseVector output) {
		...
		for (VectorEntry e : output.fast(VectorEntry.State.EITHER)) {
			...
			SparseVector itemVector = model.getItemVector(e.getKey());

			// TODO Compute the cosine of this item and the user's profile,
			// store it in the output vector
			double cosine = userVector.dot(itemVector) / (userVector.norm() * itemVector.norm());
	...
	private SparseVector makeWeightedUserVector(long user) {
		...
		// Calculated the average rating of the User.
		...
		for (Rating r : userRatings) {
			...
			// TODO Get the item's vector, convert it into Mutable copy. Then
			// multiply the Vector with the difference of rating for that item
			// and the average rating of that user and add it to the user's
			// profile
			SparseVector itemVector = model.getItemVector(r.getItemId());
			MutableSparseVector mutableItemVector = itemVector.mutableCopy();
			mutableItemVector.multiply(p.getValue() - average);
			profile.add(mutableItemVector);
		}
