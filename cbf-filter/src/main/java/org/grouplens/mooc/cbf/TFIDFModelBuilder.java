...
public class TFIDFModelBuilder implements Provider<TFIDFModel> {
	...
	public TFIDFModel get() {
		...
		for (long item : items) {
			...
			// tag on the item.
			for (String uniqTag : uniqTagsForItem) {
				if (docFreq.containsKey(tagIds.get(uniqTag))) {
					docFreq.add(tagIds.get(uniqTag), 1);
				} else {
					docFreq.set(tagIds.get(uniqTag), 1);
				}
			}
		...
		// System.out.println("itemVectors for 11L");
		// System.out.println(itemVectors.get(Long.parseLong("11")));

		// Now we've seen all the items, so we have each item's TF vector and a
		// global vector of document frequencies. Invert and log the document
		// frequency. We
		// can do this in-place.
		for (VectorEntry e : docFreq.fast()) {
			int numberOfItemsWithTag = 0;
			int totalNumberOfTags = itemVectors.size();
			Long tagId = e.getKey();

			for (Long itemId : itemVectors.keySet()) {
				MutableSparseVector tagsWithFreq = itemVectors.get(itemId);
				if (tagsWithFreq.containsKey(tagId)) {
					numberOfItemsWithTag++;
				}
			}
			...
			docFreq.set(
					e.getKey(),
					Math.log((double) totalNumberOfTags
							/ (double) numberOfItemsWithTag));
		}
